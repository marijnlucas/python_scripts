...
figure(num=None, figsize=(fig_width, fig_height), dpi=my_dpi, facecolor='w', edgecolor='k')
...
print('     '+str(nombre_courbes)+' '+extension+' files to plot')
print('     '+str(nombre_colonnes)+' columns to plot per '+extension+' file')
...
for i in tracer: 
	...
	print("     loading ", extension, " file ", liste_fichiers[i-1], " - SUCCESS (", len(donnees[tracer.index(i)]), " lines)")
...
if ('oui' in liste_offset or 'Oui' in liste_offset or 'OUI' in liste_offset or 'O' in liste_offset or 'o' in liste_offset) or ('oui' in auto_offset or 'Oui' in auto_offset or 'OUI' in auto_offset or 'O' in auto_offset or 'o' in auto_offset):
	...
	for i in tracer:
		...
		print("     adding offsets to curve ", i, " - SUCCESS")
...
######## OFFSET STATUS DISPLAYED IN TERMINAL ########	
...
if 'oui' in liste_offset or 'Oui' in liste_offset or 'OUI' in liste_offset or 'O' in liste_offset or 'o' in liste_offset:
	if 'oui' in auto_offset or 'Oui' in auto_offset or 'OUI' in auto_offset or 'O' in auto_offset or 'o' in auto_offset:
		print('     offset list mode is ACTIVE and offsets are:\n', liste_offset)
		print('\n     auto offset mode is ACTIVE (Y offsets in offset list are not considered)')
		print('\n     offset matrix:\n', offset)
		titre = titref+'\n offset: ON | auto_offset: ON'
	else:
		print('     offset list mode is ACTIVE and offsets are:\n', liste_offset)
		print('\n     auto offset mode is NOT ACTIVE')
		print('     offset matrix:\n', offset)
		titre = titref+'\n offset: ON | auto_offset: OFF'
else:
	if 'oui' in auto_offset or 'Oui' in auto_offset or 'OUI' in auto_offset or 'O' in auto_offset or 'o' in auto_offset:
		print('     offset list mode is NOT ACTIVE')
		print('     auto offset mode is ACTIVE')
		print('     offset matrix:\n', offset)
		titre = titref+'\n offset: OFF | auto_offset: ON'
	else:
		print('     offset list mode is NOT ACTIVE')
		print('     auto offset mode is NOT ACTIVE')
		print('     offset matrix:\n', offset)
...
plt.legend(fontsize=font_legend, loc='best')
plt.title(titre, fontsize=font_title)
plt.axis([xscale_low-xdelta, xscale_up+xdelta, yscale_low-ydelta, yscale_up+ydelta])
plt.xlabel(nom_axe_x, fontsize=font_axis)
plt.ylabel(nom_axe_y, fontsize=font_axis)
...
#pour permettre de créer un .png par combinaison de courbes tracées et axes représentés
	...
	for j in data_y_axis:
		titreplt+=str(j)
	titreplt+="."+image_fmt
	plt.savefig(script_dir+titreplt, figsize=(fig_width, fig_height), dpi=my_dpi, format=image_fmt)
