...
############### VERSION 1 ###############
#version1.0(2016/02/05): création du script
#version2.0(2016/02/08): fonctionne sans supprimer un pan du sweep de température
...
#partie concernant les opérations sur les courbes plotées
background=1			#courbe contenant le background
inter='linear'			#type de l'interpolation sur les données ('linear' ou 'cubic'). 'cubic' est beaucoup plus long
precision=200			#1/precision représente l'incrément en K entre chaque point (200 pour un incrément de 0.005K)
background_set_up=5		#nombre de premières lignes ignorées dans le .dat du background
background_set_low=0	#nombre de dernières lignes ignorées dans le .dat du background
...
donnees_background=import_axe_data(liste_fichiers[background-1], delimiteur, background_set_up, background_set_low, data_x_axis-1, data_y_axis-1)

######## INTERPOLATION DE BACKGROUND ########
print('where(donnees_background==min(donnees_background[:,0]))')
print(where(donnees_background==min(donnees_background[:,0])))
x_background_min=int(where(donnees_background==min(donnees_background[:,0]))[0][0])	#ligne où se situe x min de background
x_background_max=int(where(donnees_background==max(donnees_background[:,0]))[0][0])	#ligne où se situe x max de background
lines_background=shape(donnees_background)[0]

#petite vérification pour s'assurer de la bijection 'globale' de background
if int(5*lines_background/100)<=x_background_min<=int(95*lines_background/100) or int(5*lines_background/100)<=x_background_max<=int(95*lines_background/100):
	#cela signifie que la courbe background n'est probablement pas bijective
	print('####.###################################################.####')
	print('###/ \#################### WARNING ####################/ \###')
	print('##/ | \###############################################/ | \##')
	print('#/__°__\### check bijectivity of background .dat ####/__°__\#')
	print('#############################################################\n')

#on interpole sur toute la matrice donnees_background
fonction_background=interp1d(donnees_background[:,0], donnees_background[:,1], kind=inter)

######## INTERPOLATION DE TRACER ########
x_tracer_min=int(where(donnees_tracer==min(donnees_tracer[:,0]))[0][0])	#ligne où se situe x min de background
x_tracer_max=int(where(donnees_tracer==max(donnees_tracer[:,0]))[0][0])	#ligne où se situe x max de background
lines_tracer=shape(donnees_tracer)[0]

#petite vérification pour s'assurer de la bijection 'globale' de tracer
if int(5*lines_tracer/100)<=x_tracer_min<=int(95*lines_tracer/100) or int(5*lines_tracer/100)<=x_tracer_max<=int(95*lines_tracer/100):
	#cela signifie que la courbe background n'est probablement pas bijective
	print('\n####.###################################################.####')
	print('###/ \#################### WARNING ####################/ \###')
	print('##/ | \###############################################/ | \##')
	print('#/__°__\#### check bijectivity of processed .dat ####/__°__\#')
	print('#############################################################')

#on interpole sur toute la matrice donnees_tracer
fonction_tracer=interp1d(donnees_tracer[:,0], donnees_tracer[:,1], kind=inter)

######## INTERVALLES DE TRACAGE X ########
#l'intervalle de traçage est l'intersection des intervalles X de 'background' et de 'tracer'
...
x=linspace(min(intervalle_x), max(intervalle_x), num=int((max(intervalle_x)-min(intervalle_x))*precision), endpoint=True)

######## MATRICE OFFSET ########
#fabrication et initialisation matrice des offsets
offset=array(zeros((3,3)),str)	# stocke les offset de 'background' et de 'tracer'
...
#si on n'a pas de liste_offset mais qu'on a un auto_offset à mettre
...
donnees_exp_background=import_exp_data(liste_fichiers[background-1], delimiteur, background_set_up, background_set_low, col_freq, col_amp)
