...
tracer=(1,2,5,6) #pour tout tracer: 0, sinon liste de numeros (1,2,3,5,8,...) représentant l'id du graph dans la matrice offset
...
def import_axe_data(nom_fichier, data_set_up, data_set_low, data_x_axis, data_y_axis):
	# Import the data from a text file and save as a 2D matrix. Adapter le 'skip_header' au .dat
	print("---> fichier .dat en cours de traitement :", nom_fichier)
	return genfromtxt(nom_fichier, skip_header=data_set_up, skip_footer=data_set_low, usecols=(data_x_axis-1, data_y_axis-1))#stock que les données utilisées

#définition de la fonction traitement des données relatives aux conditions expérimentales:
def import_exp_data(nom_fichier, data_set_up, data_set_low, col_freq, col_amp):
	...
	matrice_donnees_exp[0] = genfromtxt(nom_fichier, skip_header=data_set_up, skip_footer=data_set_low, usecols=(col_freq-1))[0]#stock la fréquence
	matrice_donnees_exp[1] = genfromtxt(nom_fichier, skip_header=data_set_up, skip_footer=data_set_low, usecols=(col_amp-1))[0]#stock l'amplitude
...
xscale=zeros((nombre_fichiers, 2)) #matrice de stockage du X max et du X min
yscale=zeros((nombre_fichiers, 2)) #matrice de stockage du Y max et du Y min
...
for i in set(tracer): #on traite tous les fichiers distincts donnés par la liste tracer
	...
	print(i)
	donnees_axes=import_axe_data(liste_fichiers[i-1], data_set_up, data_set_low, data_x_axis, data_y_axis)
	#on importe les données des conditions expérimentales dans la matrice donnees_exp
	donnees_exp=import_exp_data(liste_fichiers[i-1], data_set_up, data_set_low, col_freq, col_amp)
	...
	if float(offset[i,1])!=0 or float(offset[i,2])!=0:
		label_courbe='graph '+str(i)+' : '+decoupage[len(decoupage)-1]+'\n ---> f='+str(donnees_exp[0,0])+'Hz | Vo='+str(donnees_exp[1,0])+'V | offset X='+offset[i,1]+' | offset Y='+offset[i,2]
	else:
		label_courbe='graph '+str(i)+' : '+decoupage[len(decoupage)-1]+'\n ---> f='+str(donnees_exp[0,0])+'Hz | Vo='+str(donnees_exp[1,0])+'V'
