...
#version5.4(20160224): ajout d'une ligne (n°) pour afficher le nombre de lignes de la matrice des données dans le terminal
...
print_graph='oui'	#'oui'/'non' pour générer ou non une copie .png
delimiteur=''		#rien par default
tracer=[1]			#tout tracer: 0, sinon liste [1,2,3,5,8,...] AVEC CROCHETS

#informations relatives aux fichiers de données
data_set_up=11	#nombre de premières lignes ignorées dans le fichier de données .dat
data_set_low=0	#nombre de dernières lignes ignorées dans le fichier de données .dat
data_x_axis=9	#colonne de données correspondant aux abscisses
data_y_axis=[1]	#colonne(s) de données correspondant aux ordonnées (garder les crochets)
error_x_axis=10		#colonne erreur sur X
error_y_axis=2		#colonne erreur sur Y
...
titref="Tc(Mo3Al2C)=f(P)"
nom_axe_x='Pressure (GPa)'
nom_axe_y='Tc Mo321 (K)'
grille='oui'
fig_width=20	#largeur fenêtre des graphes en pouces
fig_height=15	#hauteur fenêtre des graphes en pouces
font_size=18	#taille de la police des labels sur la figure des graphes
...
# paramètres relatifs à la fenêtre de traçage des graphes
figure(num=None, figsize=(fig_width, fig_height), dpi=80, facecolor='w', edgecolor='k')

# label du graph
label_graph="transition temperature of Mo321 compound at different pressures (with Tc and P measurement incertitudes)"
...
for i in tracer: 
	...
	print("     matrice contient ",len(donnees[tracer.index(i)]), " lignes.")

######## REMPLISSAGE LISTE DE MATRICES ERREUR ########
error_x_all=[]	# initialisation de la liste des matrices erreur sur x
error_y_all=[]	# initialisation de la liste des matrices erreur sur y
# autant de matrices sont crées qu'il y a de courbes à tracer
# pour accéder à la matrice d'une courbe: error_x/y[tracer.index(i)]
for i in tracer: 
	error_x_all.append(import_axe_data(liste_fichiers[i-1], delimiteur, data_set_up, data_set_low, error_x_axis-1))
	error_y_all.append(import_axe_data(liste_fichiers[i-1], delimiteur, data_set_up, data_set_low, error_y_axis-1))
...
for i in tracer:	# on traite fichier par fichier
	# on remplit xscale
	xscale[tracer.index(i),0]=min(array(donnees_off[tracer.index(i)])[:,0]-error_x_all[tracer.index(i)][:]/2)
	xscale[tracer.index(i),1]=max(array(donnees_off[tracer.index(i)])[:,0]+error_x_all[tracer.index(i)][:]/2)
	# on remplit yscale
	for j in data_y_axis:
		yscale[tracer.index(i),data_y_axis.index(j),0]=min(array(donnees_off[tracer.index(i)])[:,data_y_axis.index(j)+1]-error_y_all[tracer.index(i)][:]/2)
		yscale[tracer.index(i),data_y_axis.index(j),1]=max(array(donnees_off[tracer.index(i)])[:,data_y_axis.index(j)+1]+error_y_all[tracer.index(i)][:]/2)
...
for i in tracer:
	...
	print("matrice x :", x)
	error_x = array(error_x_all[tracer.index(i)])[:]
	print("erreur sur x :", error_x)
	error_y = array(error_y_all[tracer.index(i)])[:]
	print("erreur sur y :", error_y)
	for j in data_y_axis:
		# coordonnées en Y
		...
		plt.errorbar(x,y,xerr=[error_x/2,error_x/2], yerr=[error_y/2,error_y/2],fmt='o',label=label_graph) #on trace la courbe pour i
...
plt.xlabel(nom_axe_x, fontsize=font_size)
plt.ylabel(nom_axe_y, fontsize=font_size)
